...
public class ReefSimulator {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 10; i++) {
			...
			listOfTempOrganisms.add(new Eel(dict.generateString(), 0, 0, dict.generateInt(5), 
					"north", dict.generateColour(), dict.generateInt(15)+10));
			//goldfish have a size of 3-9
			listOfTempOrganisms.add(new Goldfish(dict.generateString(), 0, 0, dict.generateInt(5)+1, 
					"north", dict.generateColour(), dict.generateInt(6)+3));
			//Algae (example Seaweed) don't move
			listOfTempOrganisms.add(new Algae(dict.generateString(), 0, 0, 0, 
					"north", dict.generateColour(), 1));
			//sharks are much bigger and move quick
			listOfTempOrganisms.add(new Shark(dict.generateString(), 0, 0, dict.generateInt(5), 
					"north", dict.generateColour(), dict.generateInt(25)+20));
		}
		
		//Add all the Organisms in the LinkedList to the Reef
		for (Organism x : listOfTempOrganisms) {
			coralReef.addOrganism(x);
		}
